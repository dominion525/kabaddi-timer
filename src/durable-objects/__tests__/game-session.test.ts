...
describe('GameSession', () => {
  ...
  describe('ブロードキャスト動作の検証', () => {
    it('スコア更新が全クライアントにブロードキャストされる', async () => {
      ...
      expect(ws1Messages[0].type).toBe('game_state');
      expect(ws2Messages[0].type).toBe('game_state');
      expect(ws1Messages[0].data.teamA.score).toBe(5);
      expect(ws2Messages[0].data.teamA.score).toBe(5);
    });
    ...
    it('DO/DIE更新が全クライアントにブロードキャストされる', async () => {
      ...
      expect(ws1Messages[0].type).toBe('game_state');
      expect(ws2Messages[0].type).toBe('game_state');
      expect(ws1Messages[0].data.teamB.doOrDieCount).toBe(1);
      expect(ws2Messages[0].data.teamB.doOrDieCount).toBe(1);
    });
    ...
    it('チーム名変更が全クライアントにブロードキャストされる', async () => {
      ...
      expect(ws1Messages[0].type).toBe('game_state');
      expect(ws2Messages[0].type).toBe('game_state');
      expect(ws1Messages[0].data.teamA.name).toBe('東京タイガース');
      expect(ws2Messages[0].data.teamA.name).toBe('東京タイガース');
    });
    ...
    it('タイマー操作が全クライアントにブロードキャストされる', async () => {
      ...
      expect(ws1Messages[0].type).toBe('game_state');
      expect(ws2Messages[0].type).toBe('game_state');
      expect(ws1Messages[0].data.timer.totalDuration).toBe(600);
      expect(ws2Messages[0].data.timer.totalDuration).toBe(600);
    });
  });
});