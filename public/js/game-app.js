...
function gameApp(gameId) {
    ...
    return {
        ...
        get teamADoOrDieIndicators() {
            // 純粋関数を使用してインジケーター生成
            return window.ScoreLogic.generateDoOrDieIndicators(this.gameState.teamA.doOrDieCount);
        },
        get teamBDoOrDieIndicators() {
            // 純粋関数を使用してインジケーター生成
            return window.ScoreLogic.generateDoOrDieIndicators(this.gameState.teamB.doOrDieCount);
        },
        ...
        get formattedTimer() {
            // 純粋関数を使用してフォーマット
            return window.TimerLogic.formatTimer(this.timerSeconds);
        },
        get formattedSubTimer() {
            // 純粋関数を使用してフォーマット
            return window.TimerLogic.formatSubTimer(this.subTimerSeconds);
        },
        startTimer() {
        ...
        calculateTimerSeconds() {
            ...
            if (!timer)
                return;
            // 純粋関数を使用してタイマー計算
            const result = window.TimerLogic.calculateRemainingSeconds(timer, this.serverTimeOffset);
        ...
        calculateSubTimerSeconds() {
            ...
            if (!subTimer)
                return;
            // 純粋関数を使用してサブタイマー計算
            const result = window.TimerLogic.calculateSubTimerRemainingSeconds(subTimer, this.serverTimeOffset);
